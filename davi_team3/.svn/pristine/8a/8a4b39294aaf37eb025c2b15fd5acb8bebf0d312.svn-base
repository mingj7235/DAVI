...
function showReplyList (lists) {
	...
	$.each(lists, function (index, list){
			if(lists.length > 3) {
				if(index < 3) {
					textMini +='<div id = "replyContents_${certificatedList[i].getCertificatedNum()}" style="display : flex; justify-content: flex-start; height: 30px; padding: 5px;">' +
					...
					'<a href="javascript:deleteReply('+ list.certificatedNum+',' + list.certificatedReplyNum + ')"><i id="" class="far fa-minus-square icon_color" onclick=""></i></a></div></div>';
				}
