...
<html>
	<head>
		...
		<style>
			...
			#imagePreview{
				outline: 2px dashed rgba(0, 59, 251, 0.5) ;
	    		outline-offset:-10px;  
	   			text-align: center;
			  ...
			  if (files[0].type.match(/image.*/)) {
				  var reader = new FileReader();
		            ...
		            reader.onload = function(e){
		            	...
		            	let imgfile = files[0].name;
		            	$("#imgView").attr("src", imgsrc);
		            	$("#imgView").attr("data-file", imgfile);
		            };
				  $(e.target).parent().css("outline","none");
		          document.querySelector("#real-input").files = files;
			  }else{
			    alert('이미지가 아닙니다.');
			    return;
			  }
