	...
	<body class="is-preload">
		...
		<!-- Wrapper -->
			<div id="wrapper">
				...
				<form name="certificatedModifyForm">
					<div style="display: flex; justify-content: center;">
						<div style="width: 80%; height: 420px; border: 1px solid rgba(164, 167, 170, 0.3);">
							...
							<div style="display : flex; justify-content: center; height: 40px;">
								...
								<img src="${param.path}" style="display:inline-block; width:100%; height:110%; position:absolute; top:0; left:0; margin: 5px; border: 1px solid rgba(0, 59, 251, 0.3);">
								</div>
								...
								<div style="width: 70%; display:flex; justify-content: center;">
									<input type="text" class="board_content" id="certificatedTitle" name="certificatedTitle" placeholder="게시글 제목" style="border:0; border-bottom: 1px solid rgba(0, 59, 251, 0.3); 
										resize:none; height:40px; padding: 5px; margin: 5px; margin-top: 18px; margin-left: 18px;" value="${param.title}">
								</div>
							...
							<!-- 게시글 본문 -->
							<div style="display:flex; justify-content: center;">
								<textarea class="board_content" id="certificatedContent" name="certificatedContent" placeholder="게시글을 수정해보세요 :)" style="border:0; border-bottom: 1px solid rgba(0, 59, 251, 0.3); 
									resize:none; height:250px; padding: 5px; margin: 5px; margin-top:15%;">${param.content}</textarea>
							</div>
							
								<!-- 사진 업로드 -->
							<div style="display:flex; justify-content: center;">
								<input id="modifyButton" type="button" value="수정하기" style="box-shadow: none; font-size: 1em;"  >
							</div>
							<input type="hidden" name="memberId" value="${sessionScope.session_id}">
							<input type="hidden" name="certificatedNum" value="${param.num}">
		...
		/* 	function preview() {
			...
			$("#modifyButton").click(function() {
				...
				certificatedModifyForm.action = contextPath + "/board/certificateModifyOk.bo";
				certificatedModifyForm.target =  "parentPage";
				certificatedModifyForm.submit();
				self.close();
			});
